...
/** GUI for the Assembly Table, drawn with primitives (no atlas texture needed). */
public class AssemblyScreen extends AbstractContainerScreen<AssemblyMenu> {
    ...
    protected void renderBg(GuiGraphics g, float partialTick, int mouseX, int mouseY) {
        ...
        int arrowX = x + 78;
        int arrowY = y + 35;
        g.fill(arrowX, arrowY, arrowX + 24, arrowY + 16, SLOT_EDGE);
        int prog = menu.getProgressScaled(22);
        if (prog > 0) {
            g.fill(arrowX + 1, arrowY + 1, arrowX + 1 + prog, arrowY + 15, 0xFF30C0FF);
        }

        // Energy bar on the left.
        int barX = x + 14;
        int barTop = y + 18;
        int barH = 50;
    ...
    protected void renderLabels(GuiGraphics g, int mouseX, int mouseY) {
        ...
        g.drawString(font, Component.literal(menu.getEnergy() + " FE"), 28, 20, 0x404040, false);
    }
